...
export default function Dashboard({ area, setArea, complaints, onStatusUpdate }) {
  ...
  const dynamicTrendData = useMemo(() => {
    ...
    return Array.from(map.values()).sort((a, b) => a.sortKey - b.sortKey).slice(-14);
  }, [complaints]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ background: "linear-gradient(135deg, #0f172a, #1e3a5f)", borderRadius: 16, padding: "20px 24px", display: "flex", alignItems: "center", gap: 16 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ color: "#94a3b8", fontSize: 13, marginTop: 4 }}>
            System detected high volume of <b style={{ color: "#fff" }}>{typeBreakdown.length ? typeBreakdown.sort((a,b)=>b.value-a.value)[0].name : "issues"}</b> in your area. 
            Consider reallocating maintenance teams.
          </div>
        </div>
        <button 
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(380px, 1fr))", gap: 18 }}>
        ...
        <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2e8f0", padding: 22 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: "#0f172a", marginBottom: 16 }}>Filing Trend (Last 14 Days)</div>
          <ResponsiveContainer width="100%" height={280}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", minWidth: 600 }}>
          <thead>
            <tr style={{ borderBottom: "1px solid #e2e8f0" }}>
              ...
              <th style={{ padding: "12px 0", color: "#64748b", fontSize: 12, fontWeight: 600 }}>Action</th>
            </tr>
          </thead>
          <tbody>
            {recent.map(c => (
              <tr key={c.id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                ...
                <td style={{ padding: "14px 0", fontSize: 13, color: "#475569" }} title={c.desc}>{c.title ? c.title.substring(0,40) + "..." : "No Title"}</td>
